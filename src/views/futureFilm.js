...
const FutureFilm = () => {
    ...
    const [futureFilm, setFutureFilm] = useState([]);

    // lấy dữ liệu dataFilm từ backend
    useEffect(() => {
        axios.get("http://localhost:4000/api/v1/films/future-showing")
            .then(response => {
                const jsonData = JSON.parse(JSON.stringify(response.data.films));
                setFutureFilm(jsonData); // Dữ liệu đã được chuyển đổi thành đối tượng JSON
            })
            .catch(error => {
                console.error(error);
            });
    ...
    const idFilms = futureFilm.map(film => film.idFilm);

    const getFilmById = (id) => {
        return futureFilm.find(film => film.idFilm === id);
    };
                ...
                <div class="heading-content">
                    <div class="container">
                        ...
                        <h2 class="text-center">
                            <a href="" class="text-now-film">Sắp ra mắt</a>
                        </h2>
                        {/* text center */}
